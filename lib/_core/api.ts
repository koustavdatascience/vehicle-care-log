...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (Platform.OS !== "web") {
    ...
    safeDiagnostic("api.request", { method: options.method || "GET", platform: "native", hasToken: Boolean(sessionToken) });
    if (sessionToken) {
      headers["Authorization"] = `Bearer ${sessionToken}`;
    }
  } else {
    safeDiagnostic("api.request", { method: options.method || "GET", platform: "web" });
  }
  ...
  try {
    ...
    safeDiagnostic("api.response", { method: options.method || "GET", status: response.status, ok: response.ok });
    ...
    safeDiagnosticError("api.failure", error, { method: options.method || "GET" });
...
export async function exchangeOAuthCode(
  ...
  safeDiagnostic("auth.oauth_exchange", { hasSessionToken: Boolean(sessionToken), hasUser: Boolean(result.user) });
...
export async function getMe(): Promise<{
  ...
  try {
    ...
    safeDiagnosticError("auth.get_me_failure", error);
    return null;
  }
}
...
export async function establishSession(token: string): Promise<boolean> {
  try {
    ...
    if (!response.ok) {
      safeDiagnostic("auth.session_establish_failure", { status: response.status });
      return false;
    }

    safeDiagnostic("auth.session_established");
    return true;
  } catch (error) {
    safeDiagnosticError("auth.session_establish_failure", error);
    return false;
  }
}
